    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.regions.update", [$region->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('region_name') ? 'is-invalid' : '' }}" type="text" name="region_name" id="region_name" value="{{ old('region_name', $region->region_name) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pin_codes') ? 'is-invalid' : '' }}" name="pin_codes[]" id="pin_codes" multiple required>
                    @foreach($pin_codes as $id => $pin_codes)
                        <option value="{{ $id }}" {{ (in_array($id, old('pin_codes', [])) || $region->pin_codes->contains($id)) ? 'selected' : '' }}>{{ $pin_codes }}</option>
                    @endforeach
                </select>
